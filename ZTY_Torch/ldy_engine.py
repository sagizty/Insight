...
"""
我修改了一下evaluate，每个epoch保存一次，

保存一个字典

字典内容是{ index: 每个对象 } index为eval函数里面遍历的index

每个对象 是一个字典
saved_dict = {"target_bbox": targets[0]['boxes'].cpu().numpy(), "pred_bbox": p_boxes.numpy(),
                      "target_label": targets[0]['labels'].cpu().numpy(), "pred_label": names,
...
from NSCLC_script.vis import plotit
import pickle
import torch
...
def evaluate(model, criterion, postprocessors, data_loader, base_ds, device, output_dir, epoch=None):
    ...
    for index, (samples, targets) in enumerate(metric_logger.log_every(data_loader, 10, header)):
        samples = samples.to(device)
        targets = [{k: v.to(device) for k, v in t.items()} for t in targets]
        ...
        plotit(samples.tensors[0].cpu(), targets[0]['boxes'].cpu(), targets[0]['boxes'].shape[0] * ['tgt'],
